...
```{r}
occupancy$day_of_week <- lubridate::wday(occupancy$stay_date, week_start = 1)
occupancy <- occupancy |>
  mutate(weekday = ifelse(day_of_week >= 6, "Weekend", "Weekday"))
```
...
```{r}
linearModel <- lm(n ~ hotel + day_of_week + weekday, data = hotel_train) 
summary(linearModel)
```
...
booking_recipe <- occupancy %>% 
  ...
  recipe(n ~ hotel + day_of_week + weekday, data = hotel_train) %>% 
  step_date(stay_date) %>% 
...
```{r train-model1}
model1 <- hotel_lr1 %>%
  fit(formula = n ~ hotel, data = hotel_train) 
```
